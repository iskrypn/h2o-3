...
public class XGBoostScoreTask extends MRTask<XGBoostScoreTask> {
    ...
    public static float[][] scoreChunkContrib(final XGBoostModelInfo sharedmodel, final XGBoostModel.XGBoostParameters parms,
                                              final BoosterParms boosterParms, final XGBoostOutput output,
                                              final Frame fr, final Chunk[] cs) {
        ScoreResult r = scoreChunkExt(sharedmodel, parms, boosterParms, output, fr, cs, OutputType.PREDICT_CONTRIB);
        return r == null ? new float[0][] : r._preds;
    }
